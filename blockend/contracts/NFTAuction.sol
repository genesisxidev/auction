...
contract NFTAuction is ERC721A, Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    function bid(uint256 tokenId, uint256 amount) public payable nonReentrant {
        ...
        if (auctions[tokenId].highestBid == 0) {
            require(
                msg.value == amount && amount > auctions[tokenId].basePrice,
                "invalid ether sent"
            );
        } else {
            require(
        ...
        (bool success, ) = payable(currentAuctions[tokenId].previousBidder)
            .call{value: currentAuctions[tokenId].highestBid}("");
    ...
    // function transferNftsToWinners() public onlyOwner returns (bool) {
    //     //require(readyToMint, "satisfy all auctions before transferring");
    //     require(checkIfAllAuctionsAreEnded(), "not all auctions are ended");
    //     return transferNFTs();
    // }
    ...
    function checkIfAllAuctionsAreEnded() internal view returns (bool) {
        if (nextTokenId > 10) {
            for (uint256 i = nextTokenId - 11; i < nextTokenId - 1; i++) {
                if (auctions[i].status != status.Ended) {
                    return false;
                }
            }
        } else {
        ...
        returns (AuctionPayload[] memory)
    {
        ...
        AuctionPayload[] memory upcomingAuctions = new AuctionPayload[](count);
        uint256 index = 0;
        for (uint256 i = 1; i <= nextTokenId; i++) {
            if (auctions[i].startTime > block.timestamp) {
                upcomingAuctions[index] = AuctionPayload({
    ...
    function getLiveAuctions() public view returns (AuctionPayload[] memory) {
        ...
        for (uint256 i = 1; i <= nextTokenId; i++) {
            if (
                auctions[i].startTime < block.timestamp &&
